...
def main():
    ...
    for _ in range(config.MAX_RETRY_COUNT):
        try:
            resources = reset.reset(orchestrator_connection)
            ...
            while task_count < config.MAX_TASK_COUNT:
                ...
                try:
                    process.process(orchestrator_connection, queue_element, **resources)

                except BusinessError as error:
                    orchestrator_connection.set_queue_element_status(queue_element.id, QueueStatus.FAILED, message=str(error))
    ...
    if config.FAIL_ROBOT_ON_TOO_MANY_ERRORS and too_many_errors:
